module.processor.ProcessorRunner.runDayByDay{
	startYear 2001//Calendar.getInstance().get(Calendar.YEAR)
	startMonth 1//Calendar.getInstance().get(Calendar.MONTH)+1
	...
	process{yyyyMmDd->
	    if(new File('./margin_transactionl/'+yyyyMmDd+'_summary.sql').exists() && new File('./margin_transactionl/'+yyyyMmDd+'_all.sql').exists()){
	    	print '>'
	    }else{
			    ...
			    if(resultSql && !resultSql.endsWith('VALUES ')){
			    	new File('./margin_transactionl/').mkdir()
		     		new FileOutputStream('./margin_transactionl/'+yyyyMmDd+'_all.tmp').withWriter('UTF-8') { writer ->
		        		writer << resultSql+';'
		     		}
		     		new File('./margin_transactionl/'+yyyyMmDd+'_all.tmp').renameTo('./margin_transactionl/'+yyyyMmDd+'_all.sql')
					print '#'
				}
			    ...
			    if(resultSqlSummary && !resultSqlSummary.endsWith('VALUES ')){
			    	new File('./margin_transactionl/').mkdir()
		     		new FileOutputStream('./margin_transactionl/'+yyyyMmDd+'_summary.tmp').withWriter('UTF-8') { writer ->
		        		writer << resultSqlSummary+';'
		     		}
		     		new File('./margin_transactionl/'+yyyyMmDd+'_summary.tmp').renameTo('./margin_transactionl/'+yyyyMmDd+'_summary.sql')
					print '*'
				}
	    }
	}
...
module.db.SqlExecuter.execute{
    dir './margin_transactionl'
}
module.io.FileBetch.execute{
	clean './margin_transactionl'
	delete './margin_transactionl'
}
println 'import margin_transactionl done'